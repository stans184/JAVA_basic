...
public class AnimalTest {
    ...
    public void testDownCasting(ArrayList<Animal> testList){
        for(int i=0;i<testList.size();i++){
            Animal animal = testList.get(i);

            if (animal instanceof Human){
                Human human = (Human)animal;        // animal 로 생성된 instance를 human 으로 downcasting
                human.read();                       // 할 수 있으면 다형성을 이용해서
            }                                       // 반드시 돌려야 할 경우 아니면,,,굳이?
